...
\texttt{\{le.nguyen, alex.poldrugo, \& rafidison.rakotondrasoa\}.001@student.uni.lu} \\ %in case your adding Lu email address use this template and if you want to add different email address from your coauthor use the following template. 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=4cm, height=6cm]{4.png}
...
We used ensemble methods for classification. Ensemble methods are techniques that create multiple models and then combine them to produce improved results. We can combine any machine learning algorithm such as logistic regression, decision tree, naive bayes. These models, when used as inputs of ensemble methods, are called "weak learners" or ”base models”. Some widely known methods of ensemble: bagging, boosting and stacking.

\subsubsection{Bagging}

Bagging or Bootstrap Aggregation is an ensemble learning technique which perform a set of homogeneous weak learners. Homogeneous means that we have \emph{n} numbers of the same learner and not different kinds of learners. The bagging process is, first, to get \emph{n} subsets from the training, learns them independently from each other in parallel and combines them following some kind of deterministic averaging process.

\subsubsection{Boosting}

Boosting refers to the process of sequentially training some homogeneous base models to build a model. Here, a base model depends on the previous ones. Predictions for boosting algorithms usually assign weights to determine the importance of each learner’s input. The boosting process is, first, to train one base model, evaluate where the model or learner misclassified, then train another learner focusing on the areas where the previous learner misclassified and repeat this process until some stopping criteria is met, e.g., performance plateau.

\subsubsection{Stacking}

Stacking considers heterogeneous weak learners. Heterogeneous means that we have different sort of learners (e.g. Decision tree, KNeighbors, Naive Bayes, etc). Stacking learns them in parallel and combines them by training a another leanrer called "meta-model" to output a prediction based on the different weak learners predictions.
...
We executed a grid search for both sentiment and topic classification tasks. The best hyperparameters for the sentiment and topic classification task can be consulted in the Appendix in Table \ref{tab:sentiment_gs_hp} and \ref{tab:topic_gs_hp} respectively. The scores can be consulted in the Appendix in Table \ref{tab:sentiment_gs} and \ref{tab:topic_gs}. The \textit{best\_score} in these tables represent the training accuracies.

\subsubsection{Grid Search: Ensemble methods}

The best hyperparameters of the ensemble methods for the sentiment and topic classification task can be consulted in the Appendix in Table \ref{tab:sentiment_em_gs_hp} and \ref{tab:topic_em_gs_hp} respectively. The scores can be consulted in the Appendix in Table \ref{tab:sentiment_em_gs} and \ref{tab:topic_em_gs}. The \textit{best\_score} in these tables represent the training accuracies.

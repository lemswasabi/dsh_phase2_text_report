...
%\vspace{0pt}
\\
\textsuperscript{1}0180531722,  \textsuperscript{2} Your student number \textsuperscript{3} 0190252530\\
\texttt{\{le.nguyen.001, alex.podrugo.001, \& rafidison.rakotondrasoa.001\}@student.uni.lu} \\ %in case your adding Lu email address use this template and if you want to add different email address from your coauthor use the following template. 
...
Before we can explore the dataset and visualize it, we first need to transform the dataset into a representation that we can work with. The given dataset is stored in a \textit{.txt} file. Each row of this file contains a review sample which is delimited by a newline character. Each sample is divided into sections. The first word represents the topic of the review, whereas the second word depicts its sentiment. The third word of the sample serves as id. The rest of the row presents the review text itself. This is visualized in Fig. \ref{fig:review_sample}
...
\begin{figure}[ht]
    ...
    \caption{\scriptsize Distribution of sentiment.}
...
\begin{figure}[ht]
    ...
    \caption{\scriptsize Distribution of topic.}
...
\begin{figure}[ht]
    ...
    \caption{\scriptsize Distribution of sentiment per topic.}
...
\begin{figure}
    ...
    \caption{\scriptsize text length of our reviews}
...
The figure \ref{fig:countvect} shows how a text will be represented.

\begin{figure}[h!]
    \centering
    \includegraphics[width=6cm, height=4cm]{1.png}
    \caption{\scriptsize Count Vectorizer}
    \label{fig:countvect}
\end{figure}
...
The figure \ref{fig:tfidfvect} shows how a text will be represented.

\begin{figure}[h!]
    \centering
    \includegraphics[width=7cm, height=4cm]{2.png}
    \caption{\scriptsize TF-IDF Vectorizer}
    \label{fig:tfidfvect}
\end{figure}
...
We use VarianceThreshold to do feature selection on our sample vectors. By default, it removes all zero-variance features, i.e. features that have the same value in all samples (partially similar to the TF-IDF features creation).

The figure \ref{fig:removfeatures} shows the results after selecting features.

\begin{figure}[h!]
    \centering
    \includegraphics[width=6cm, height=8cm]{3.png}
    \caption{\scriptsize Removing zero-variance features}
    \label{fig:removfeatures}
\end{figure}

\subsubsection{Univariate feature selection}

We perform a Chi-Square test to the samples to retrieve only the k best features.

The figure \ref{fig:univfeature} shows the results after selecting features.

\begin{figure}[h!]
    \centering
    \includegraphics[width=5cm, height=7cm]{4.png}
    \caption{\scriptsize Univariate feature selection that retrieves the 4000 best features}
...
A visualization of K-Fold, an example of cross-validation, is given in Fig. \ref{fig:cross_validation}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{cross_validation.png}
    \caption{\scriptsize K-fold example where $k = 5$.}
...
\begin{table}[!ht]
\centering
\caption{\scriptsize Sentiment classification final assessment\label{tab:sentimentassessment}}
\small\addtolength{\tabcolsep}{-4pt}
\resizebox{0.8\linewidth}{!}{
\begin{tabular}{|l|l|l|}
\hline
...
\begin{table}[!ht]
\centering
\caption{\scriptsize Topic classification final assessment\label{tab:topicassessment}}
\small\addtolength{\tabcolsep}{-4pt}
\resizebox{0.8\linewidth}{!}{
\begin{tabular}{|l|l|l|}
\hline
...
\begin{table}[!ht]
\centering
\caption{\scriptsize Best hyperparameters found during grid search for the sentiment classification task.}
...
\begin{table}[!ht]
\centering
\caption{\scriptsize Best hyperparameters found during grid search for the topic classification task.}
\label{tab:topic_gs}
\small\addtolength{\tabcolsep}{-4pt}
\resizebox{\linewidth}{!}{%
\begin{tabular}{|c|c|c|c|}
...
\hline
classifier & best\_params & best\_score & std\_test\_score \\ \hline
BaggingClassifier & \{'base\_estimator': SVC(C=10, class\_weight='balanced', probability=True), 'max\_samples': 500, 'n\_estimators': 80\}	& 0.807	& 0.031 \\ \hline
ExtraTreesClassifier & \{'class\_weight': None, 'criterion': 'entropy', 'max\_samples': 400, 'n\_estimators': 90\}	& 0.820	& 0.022 \\ \hline
GradientBoostingClassifier & \{'learning\_rate': 0.3, 'n\_estimators': 120\}	& 0.784 & 0.013 \\ \hline
StackingClassifier & \{'final\_estimator': LogisticRegression(solver='liblinear')\}	& 0.822	& 0.019 \\ \hline
XGBClassifier & \{'learning\_rate': 0.3, 'n\_estimators': 130\} & 0.794 & 0.023 \\ \hline
\end{tabular}
%
}
\end{table}
...
\hline
classifier & best\_params & best\_score & std\_test\_score \\ \hline
BaggingClassifier & \{base\_estimator=SVC(C=10, class\_weight='balanced', probability=True), max\_samples=500, n\_estimators=80\}	& 0.907	& 0.009 \\ \hline
ExtraTreesClassifier & \{'class\_weight': 'balanced', 'criterion': 'gini', 'max\_samples': 200, 'n\_estimators': 90\}	& 0.898	& 0.014 \\ \hline
GradientBoostingClassifier & \{'learning\_rate': 0.2, 'n\_estimators': 120\}	& 0.861	& 0.015 \\ \hline
StackingClassifier & \{'final\_estimator': LogisticRegression(solver='liblinear')\} & 0.925 & 0.006 \\ \hline
XGBClassifier & \{'learning\_rate': 0.3, 'n\_estimators': 130\}	& 0.886	& 0.016 \\ \hline
\end{tabular}
%
}
\end{table}
